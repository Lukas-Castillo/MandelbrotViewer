...
void main (void) {
    ...
    dvec2 zPrev = dvec2(0.0lf, 0.0lf); // Z_0 = 0, matches direct-mode's z starting at 0

    for (int i = 0; i < u_recLimit; i++) {
        if (i >= u_orbitLength) break;

        // Use Z_i (the PREVIOUS fetch result) for this step's multiplication
        dz = 2.0lf*dvec2(zPrev.x*dz.x - zPrev.y*dz.y, zPrev.x*dz.y + zPrev.y*dz.x)
            + dvec2(dz.x*dz.x - dz.y*dz.y, 2.0lf*dz.x*dz.y)
            + dz0;
        count++;

        dvec2 Zi1 = fetchOrbit(i);   // this is Z_{i+1}
        z = dz + Zi1;
        zPrev = Zi1;                 // becomes "Z_i" for the NEXT loop iteration
    ...
    if(pertube){
        if(count1 == count) gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0);
